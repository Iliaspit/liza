...
func buildInstructionsForWakeTrigger(wakeTrigger, agentID string, wakeData wakeTemplateData, planningTasks []planningTaskData) (string, error) {
	...
	case "GRAPH_REPLAN_REQUEST":
		return fmt.Sprintf("Claim graph re-plan request %s with `%s claim-graph-replan %s --agent-id %s --json`. Re-read durable tasks, candidates, and evidence; diagnose the graph fault; use only Lisa-native task operations for the smallest in-scope correction. Then run `%s complete-graph-replan %s --diagnosis \"...\" [--updates-file <json>] --agent-id %s --json`. If claim or completion reports that the frozen graph or candidate identity is stale while scope and acceptance remain unchanged, do not retry the unchanged operation; report the exact request ID so the controller can use `refresh-graph-replan`. Refuse product, scope, or acceptance changes. Current diagnostic: %s", wakeData.GraphReplanRequestID, promptBinaryName(), wakeData.GraphReplanRequestID, agentID, promptBinaryName(), wakeData.GraphReplanRequestID, agentID, wakeData.GraphReplanDiagnostic), nil
