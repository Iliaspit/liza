...
func TestUpdate_AKeyIsIgnored(t *testing.T) {
	m := testModel()
	...
	if m2.inputMode != InputModeNormal {
		t.Errorf("pressing a changed inputMode to %d, want InputModeNormal(%d)", m2.inputMode, InputModeNormal)
	}
	if cmd != nil {
		t.Error("pressing a should not return a command")
	}
}
