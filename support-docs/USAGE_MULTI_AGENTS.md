...
`codex-acp` is an opt-in ACPX-backed Codex runtime. It requires `acpx` on `PATH`, uses Codex's `AGENTS.md` contract setup, and runs ACPX non-interactively with auto-approved provider prompts inside Liza's supervised task worktree. Unlike the CLI-backed runtimes, `codex-acp` currently does not write `.liza/agent-outputs/` transcript files; ACP output is returned to the supervisor and lifecycle/usage metadata is logged.
