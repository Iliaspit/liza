...
`codex-acp` is an opt-in ACPX-backed Codex runtime. It requires `acpx` on `PATH`, uses Codex's `AGENTS.md` contract setup, and runs ACPX non-interactively with auto-approved provider prompts inside Liza's supervised task worktree. During `acpx prompt`, stdout JSON-RPC and stderr diagnostics are streamed to `.liza/agent-outputs/`; parsed message chunks are returned to the supervisor and lifecycle/usage metadata is logged. Short ACPX session control calls are not transcript-logged.
