...
model-specific blind spots. The human remains the approval authority; the
blackboard coordinates doer/reviewer state, submitted artifacts, review notes,
validation output, and decisions.

The doer session is the only coding/control session and still stops at normal
Pairing approval gates before state changes. Reviewer sessions run autonomously:
they read the blackboard, inspect submitted artifacts, and write review notes or
verdicts without asking the human for each review action.
