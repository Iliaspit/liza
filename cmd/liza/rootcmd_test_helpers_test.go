...
func resetRootCmdForTest(t *testing.T) {
	...
	for _, child := range rootCmd.Commands() {
		...
		for _, name := range []string{"spec", "config", "entry-point", "branch", "post-worktree-cmd", "copy-worktree-env-files", "auto-resume", "no-follow-up", "default-cli", "default-doer-cli", "default-reviewer-cli", "scip-search", "cli", "claude", "codex", "gemini", "mistral"} {
			resetFlagIfPresent(child, name)
		}
