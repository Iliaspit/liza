...
**done_when:** Unit tests in `internal/scipsearch` prove runtime target selection is a no-op when `LIZA_ENABLE_SCIP_SEARCH` is false or `config.scip_search` is empty, includes only languages that are both git-detected in the target root and configured, preserves deterministic `go`, `typescript`, `python` order, constructs absolute `<target>/.liza/scip/<language>.scip` output paths, and produces exact argv/working-directory command plans for `scip-go index --module-root <target> --output <path>`, `scip-typescript index --cwd <target> --output <path> <target>`, and `scip-python index --cwd <target> --output <path>`.
