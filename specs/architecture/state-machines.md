...
`BLOCKED` is not a generally claimable pipeline state and must not be wired as a broad pipeline transition back to an initial or ready status. The only supported exception is the `unblock-task` operation. After dependency validation, optional preserved-worktree validation, and optional unblock-time rebase validation, unassigned `unblock-task` may move a repaired task with valid pending dependencies from `BLOCKED` to that task's role-pair initial status. That restored task remains dependency-held and unclaimable until every direct dependency is `MERGED`. With `--assign-to`, the same operation may directly resume the role-pair executing status for a registered doer only when every dependency is satisfied; direct assignment remains rejected while any dependency is unmet.

A role-pair initial task with `worktree` still set is a preserved-branch continuation, not stale state. Claiming that shape first rechecks dependencies, captures the current integration commit, and rebases or validates the preserved HEAD on that captured integration SHA. The completion lock then encloses the final integration-ref equality check and assignment so cooperating integration movement is ordered before or after the assignment, without holding the integration mutation lock across the blackboard write. If preservation validation or freshness fails, claim must fail rather than silently deleting or replacing the worktree.
...
| BLOCKED → role-pair initial (`unblock-task`) | status=role-pair initial; clear `assigned_to`, `lease_expires`, blocked metadata | valid preserved `worktree`, `base_commit` when present | Operation-scoped exception only; valid pending dependencies may remain, so the task can be dependency-held rather than immediately claimable. If `--rebase-on` succeeds, set `base_commit` to the resolved target SHA. |
| BLOCKED → role-pair executing (`unblock-task --assign-to`) | status=role-pair executing; `assigned_to`, `lease_expires` | valid preserved `worktree`; `base_commit` updated by successful `--rebase-on` | Direct-resume fast path for a registered doer with matching role, no conflicting current task, and every dependency satisfied; no live PID proof required. |
