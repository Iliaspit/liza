...
Agent status dots use colors from the unified palette above. Filled ● for active statuses, hollow ○ for idle/waiting. The CURRENT_TASK column uses the remaining row width after fixed columns and truncates only when the task ID exceeds that available space.
...
The ID column prefers 72 cells (80% wider than the original 40-cell column) and shrinks only when fixed columns plus minimum flexible columns would otherwise exceed the panel width.
